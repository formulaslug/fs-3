...
bool d6t_1a_checkPEC(uint8_t buf[], int n) {
    int i;
    uint8_t crc = d6t_1a_calc_crc((D6T_ADDR << 1) | 1);  // I2C Read address (8bit)
    for (i = 0; i < n; i++) {
        crc = d6t_1a_calc_crc(buf[i] ^ crc);
    }
    bool ret = crc != buf[n];
...
void d6t_1a_loop() {
    ...
    d6t_1a_checkPEC(rbuf, N_READ - 1);

    //Convert to temperature data (degC)
    d6t_1a_ptat = (double)d6t_1a_conv8us_s16_le(rbuf, 0) / 10.0;
	for (i = 0; i < N_PIXEL; i++) {
		itemp = d6t_1a_conv8us_s16_le(rbuf, 2 + 2*i);
