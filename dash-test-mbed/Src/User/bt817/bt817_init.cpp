...
void bt817_init(void) {
	...
	while (0x7C != rd8(REG_ID)) {
		timeout++; // empty while to pause execution until the right value
		         // is returned
	    if (timeout> 250) {
	        // DBG_A("After 250 tries, 0x7c not returned from BT817. Check connection\r\n");
	        timeout = 0;
	        // HAL_Delay(10000);
	    }
	}
	while (0x0 != rd16(REG_CPURESET)) {
		timeout ++;
	    if (timeout > 250) {
	        // DBG_A("After 250 tries, 0x00 not returned from CPU reset. Hardware error?\r\n");
	        timeout = 0;
	        // HAL_Delay(10000);
	    }
	}
    ...
    // DBG_A("BT817 frequency set to 72MHZ \r\n");
    ...
    // DBG_A("BT817 LCD Parameters Set\r\n");
    ...
    // DBG_A("BT817 pin drive set\r\n");

	wr16(REG_ADAPTIVE_FRAMERATE, 0);
    wr8(REG_VOL_PB, 0);
    wr32(REG_PLAYBACK_PLAY, 0);
    wr8(REG_VOL_SOUND, 0);
    wr16(REG_SOUND, 0);
