...
int main(int argc, char *argv[]) {
    ...
    int sd_error = fs.mount(&sd);
    if (sd_error) {
        // Reformat if we can't mount the filesystem.
        // This should only happen on the first boot
        printf("No filesystem found, formatting...\n");
        sd_error = fs.reformat(&sd);
        if (sd_error) error_quit("Error: could not reformat SD card! Is the SD card plugged in?\n");
    }
    ...
    ArrowSchemaInit(&schema_root);
    // ArrowSchemaSetTypeStruct(schema_root.get(), COLS);
    ArrowSchemaSetTypeStruct(&schema_root, 100);
    ...
    ArrowArrayInitFromType(&array_root, NANOARROW_TYPE_STRUCT);

    printf("2.5\n");

    // ArrowArrayAllocateChildren(array_root.get(), COLS);
    ArrowArrayAllocateChildren(&array_root, 100);
    ...
    ArrowArrayStartAppending(&array_root);
    ...
    ArrowArrayReserve(&array_root, 5);
    ...
    // FIXME: Should only be necessary to call this on the root!!!
    ARROW_ERROR_PRINT(ArrowArrayFinishBuilding(&array_root, NANOARROW_VALIDATION_LEVEL_MINIMAL, &arrow_error));
    ...
    ARROW_ERROR_PRINT(ArrowArrayViewInitFromSchema(array_view.get(), &schema_root, &arrow_error));
    ARROW_ERROR_PRINT(ArrowArrayViewSetArray(array_view.get(), &array_root, &arrow_error));
    ...
    ARROW_ERROR_PRINT(ArrowIpcWriterStartFile(ipc_writer.get(), &arrow_error));
    ARROW_ERROR_PRINT(ArrowIpcWriterWriteSchema(ipc_writer.get(), &schema_root, &arrow_error));
    ...
    for (int i = 0; i < 1; i++) {
        ARROW_ERROR_PRINT(ArrowIpcWriterWriteArrayView(ipc_writer.get(), array_view.get(), &arrow_error));
    }
    ...
    ARROW_ERROR_PRINT(ArrowIpcWriterFinalizeFile(ipc_writer.get(), &arrow_error));
